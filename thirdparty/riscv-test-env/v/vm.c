...
{
  *(volatile char *)CONSOLE_BASE = x & 0xFF;
}
...
static void shutdown(int code, int status)
{
  volatile uint32_t* const power_off_reg = (volatile uint32_t*)TEST_BASE;
  *power_off_reg = ((uint32_t)code << 16) | status;
}

static void terminate(int code)
{
  shutdown(code, 0x3333);
  while (1);
}
...
void handle_trap(trapframe_t* tf)
{
  ...
  {
    cputstring("illegal instruction\n");
...
# error
  ...
  user_llpt[TEST_BASE >> PGSHIFT] = ((pte_t)TEST_BASE >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_A | PTE_D;
  uintptr_t console_aligned = CONSOLE_BASE & ~((uintptr_t)MEGAPAGE_SIZE - 1);
  user_l2pt[CONSOLE_BASE >> 21] = ((pte_t)console_aligned >> PGSHIFT << PTE_PPN_SHIFT) | PTE_V | PTE_R | PTE_W | PTE_A | PTE_D;
