...
class uemu(pluginTemplate):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.dut_exe = os.path.join(config["PATH"] if "PATH" in config else "", "uemu")

        # Number of parallel jobs that can be spawned off by RISCOF
        # for various actions performed in later functions, specifically to run the tests in
        # parallel on the DUT executable. Can also be used in the build function if required.
        self.num_jobs = str(config["jobs"] if "jobs" in config else 6)
    ...
    def build(self, isa_yaml, platform_yaml):
        ...
        # for uemu start building the '--isa' argument. the self.isa is dutnmae specific and may not be
        # useful for all DUTs
        ...
        self.compile_cmd = (
            ...
            + (
                ...
                else ("ilp32e " if "E" in ispec["ISA"] else "ilp32 ")
            )
        )
    ...
    def runTests(self, testList):
        ...
        for testname in testList:
            ...
            if self.target_run:
                timeout = 4000
                simcmd = "{0} -f {1} -s {2} -t {3} -m 128 --headless".format(
                    self.dut_exe, elf, sig_file, timeout
                )
            else:
                simcmd = 'echo "NO RUN"'
