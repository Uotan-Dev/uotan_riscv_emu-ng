...
class sail_cSim(pluginTemplate):
    ...
    def runTests(self, testList, cgf_file=None, header_file=None):
        ...
        if "PMP" in isa_yaml["hart0"]:
            pmp_node = isa_yaml["hart0"]["PMP"]

            if pmp_node.get("implemented", False):
                if "pmp-grain" in pmp_node:
                    pmp_grain = pmp_node["pmp-grain"]
                else:
                    logger.error("PMP grain not defined in YAML, defaulting to 0")
                if "pmp-count" in pmp_node:
                    pmp_count = pmp_node["pmp-count"]
                else:
                    logger.error("PMP count not defined in YAML, defaulting to 0")
        ...
        sail_config["memory"]["pmp"]["grain"] = pmp_grain
        sail_config["memory"]["pmp"]["count"] = pmp_count
